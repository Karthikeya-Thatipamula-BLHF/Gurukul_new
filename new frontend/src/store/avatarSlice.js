...
const initialState = {
  ...
  // Custom Images state
  customImages: [], // Array of uploaded images with metadata
...
export const avatarSlice = createSlice({
  ...
  reducers: {
    ...
    setFavorites: (state, action) => {
      ...
      // Save favorites to localStorage immediately for persistence across logout/login
      try {
        ...
        console.error("Error saving favorites after setting:", error);
      }
    },
    addFavorite: (state, action) => {
      ...
      try {
        ...
        console.error("Error saving favorites after adding:", error);
      }
    },
    removeFavorite: (state, action) => {
      ...
      try {
        ...
        console.error("Error saving favorites after removing:", error);
      }
    },
    updateFavorite: (state, action) => {
      ...
      if (index !== -1) {
        ...
        try {
          ...
          console.error("Error saving favorites after updating:", error);
        }
      }
    },
    ...
    setSelectedAvatar: (state, action) => {
      ...
      // Save the selected avatar ID to localStorage for persistence across logout/login
      if (action.payload && action.payload.id) {
        try {
          ...
          console.error("Error saving last selected avatar ID:", error);
        }
      }
    },
    ...
    setIsPinModeEnabled: (state, action) => {
      ...
      console.log("🎭 Redux: Setting pin mode enabled:", {
        from: state.isPinModeEnabled,
        to: newPinModeState,
      });
    ...
    setCustomImagesLoading: (state, action) => {
      state.customImagesLoading = action.payload;
    },
    setCustomImagesError: (state, action) => {
      state.customImagesError = action.payload;
    },
    addCustomImage: (state, action) => {
      state.customImages.push(action.payload);
    },
    ...
    loadCustomImages: (state, action) => {
      state.customImages = action.payload;
    },
    ...
    loadAvatarSettings: (state, action) => {
      ...
      state.pinPosition =
        avatar.pinPosition || avatar.avatarPosition || defaultPosition;
      ...
      state.isPinModeEnabled =
        avatar.isPinModeEnabled !== undefined ? avatar.isPinModeEnabled : true; // Default to enabled
    ...
    autoSaveAvatarSettings: (state) => {
      ...
      try {
        ...
        // Save favorites to localStorage for persistence across logout/login
        try {
          localStorage.setItem(
            "gurukul_favorite_avatars",
            JSON.stringify(state.favorites)
        ...
        // Also save global state (pin settings, etc.) for persistence
        try {
          const globalState = {
            isPinModeEnabled: state.isPinModeEnabled,
            pinnedAvatarPosition: state.pinnedAvatarPosition,
          ...
          console.error(
            "Error saving global state to localStorage:",
            globalStateError
          );
        }
      } catch (error) {
    ...
    loadGlobalState: (state, action) => {
      ...
      state.isPinModeEnabled =
        globalState.isPinModeEnabled !== undefined
          ? globalState.isPinModeEnabled
    ...
    addChatMessage: (state, action) => {
      ...
      const existingMessage = state.chatHistory.find(
      ...
      if (!existingMessage) {
        ...
        if (newMessage.isWelcome && newMessage.role === "assistant") {
          const existingWelcome = state.chatHistory.find(
            (msg) =>
              msg.isWelcome &&
              msg.role === "assistant" &&
        ...
        if (
          newMessage.model === "system" &&
          newMessage.role === "assistant" &&
          !newMessage.isWelcome
        ) {
...
export const selectCustomImages = createSelector(
  [selectAvatarState],
  (avatarState) => avatarState.customImages
);

export const selectCustomImagesLoading = createSelector(
  [selectAvatarState],
  (avatarState) => avatarState.customImagesLoading
);

export const selectCustomImagesError = createSelector(
  [selectAvatarState],
  (avatarState) => avatarState.customImagesError
);
...
export const selectHasUnsavedChanges = createSelector(
  ...
  (avatar) => {
    ...
    const savedPinMode =
      avatar.selectedAvatar.isPinModeEnabled !== undefined
        ? avatar.selectedAvatar.isPinModeEnabled

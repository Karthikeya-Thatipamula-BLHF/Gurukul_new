...
export const chatApiSlice = createApi({
  ...
  endpoints: (builder) => ({
    ...
    sendChatMessage: builder.mutation({
      query: ({ aiMessage, userQuery, userId, llmModel = "uniguru" }) => ({
...
export const coreApiSlice = apiSlice.injectEndpoints({
  endpoints: (builder) => ({
    ...
    checkSimulationStatus: builder.query({
      query: ({ taskId, userId }) =>
        `/simulation-status/${taskId}?user_id=${userId || "guest-user"}`,
    ...
    checkLearningTaskStatus: builder.query({
      query: ({ taskId, userId }) =>
        `/user/learning/${taskId}?user_id=${userId || "guest-user"}`,
      providesTags: (result, error, { taskId }) => [
...
export const { useSendChatMessageMutation, useLazyFetchChatbotResponseQuery } =
  chatApiSlice;

...
const AvatarModel = memo(function AvatarModel({
  ...
  // Track default avatar loading
  useEffect(() => {
    if (isDefaultAvatar) {
      // Default avatar loading tracking (logging disabled for cleaner console)
    }
  ...
  useEffect(() => {
    if (availableAnimations.length > 0) {
      // Animation tracking (logging disabled for cleaner console)
    }
  ...
  useFrame((state) => {
    if (meshRef.current && meshRef.current.position && meshRef.current.rotation && meshRef.current.scale) {
      // Update position - ensure it's applied every frame
      const [x, y, z] = basePosition.current;
      meshRef.current.position.set(x, y, z);

      // Update scale - ensure it's applied every frame
      const scaleValue = baseScale.current;
      meshRef.current.scale.set(scaleValue, scaleValue, scaleValue);

      // Update rotation
      if (autoRotate) {
        // Apply gentle auto-rotation on top of base rotation for avatars
        const [rx, ry, rz] = baseRotation.current;
        meshRef.current.rotation.x = rx;
        meshRef.current.rotation.y = ry + Math.sin(state.clock.elapsedTime * 0.3) * 0.05;
...
const AvatarViewer = memo(function AvatarViewer({
  ...
  // Track avatar loading (logging disabled for cleaner console)
  useEffect(() => {
    // Avatar loading tracking

...
export function useAvatarAnimations(gltf, meshRef, options = {}) {
  ...
  if (!gltf || !meshRef) {
    if (debug)
      console.log(
        "useAvatarAnimations: Missing gltf or meshRef, returning safe defaults"
      );
    return {
  ...
  const { actions, names, clips } = useAnimations(
    gltf?.animations || [],
    meshRef
  );
  ...
  useEffect(() => {
    ...
    if (!hasAnimations) {
      if (debug)
        console.log("No GLTF animations found, using procedural animations");
      ...
      if (enableProceduralAnimations) {
        ...
        if (debug)
          console.log("Using procedural animations:", proceduralAnimations);
      }
      return;
    }
    ...
    if (debug) {
      // Animation debugging (logging disabled for cleaner console)
    }
  ...
  const playAnimation = useCallback(
    (animationName, options = {}) => {
      ...
      if (enableProceduralAnimations) {
        const animDuration =
          options.duration || (animationName === "idle" ? 2000 : 1000);
        return playProceduralAnimation(animationName, animDuration);
      }

      if (debug)
        console.warn(
          `Animation "${animationName}" not found and procedural animations disabled`
        );
      return false;
    },
    [
  ...
  const playIdleAnimation = useCallback(() => {
    if (hasGLTFAnimations && idleActionRef.current) {
      const idleName = availableAnimations.find(
        (anim) => actions[anim.name] === idleActionRef.current
      )?.name;
  ...
  const getRandomAnimationByType = useCallback(
    (type) => {
      const animsOfType = availableAnimations.filter(
        (anim) => anim.type === type
      );
  ...
  const handleMouseEnter = useCallback(
    (event) => {
      ...
      if (debug) {
        // Mouse interaction debugging (logging disabled for cleaner console)
      }
    },

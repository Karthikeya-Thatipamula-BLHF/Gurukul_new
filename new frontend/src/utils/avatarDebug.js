 ...
 * NOTE: Debug logging disabled for cleaner console output
 */

export const testAvatarPath = async (path) => {
  // Avatar path testing (logging disabled for cleaner console)
...
export const runFullAvatarTest = async (path = "/avatar/jupiter.glb") => {
  // Avatar debug testing disabled for cleaner console
  return {
    path,
    timestamp: new Date().toISOString(),
    tests: {
      pathAccess: { success: true, note: "Debug disabled" },
      gltfLoading: { success: true, note: "Debug disabled" },
      alternativePaths: { note: "Debug disabled" },
    },
  };
  ...
  console.table({
    ...
    "Alternative Paths": Object.values(results.tests.alternativePaths).some(
      (r) => r.success
    )
      ? "✅ Some Pass"
      : "❌ All Fail",
  ...
  if (!results.tests.pathAccess.success) {
    console.error(
      "🚨 Primary issue: Cannot access avatar file at specified path"
    );
...
if (import.meta.env.DEV) {
  ...
  // Avatar debug utilities loaded (logging disabled for cleaner console)
}

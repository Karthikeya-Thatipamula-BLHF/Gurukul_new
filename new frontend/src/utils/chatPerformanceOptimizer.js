...
class ChatPerformanceOptimizer {
  ...
  optimizeMessagesForRendering(messages, currentIndex = 0) {
    if (
      ...
      messages.length <= PERFORMANCE_THRESHOLDS.MAX_RENDERED_MESSAGES
    ) {
      return messages;
    }
  ...
  getPerformanceRecommendations(messageCount, storageSize) {
    ...
    if (messageCount >= PERFORMANCE_THRESHOLDS.HUGE_HISTORY) {
      recommendations.push({
        type: "warning",
        message:
          "Large chat history detected. Consider exporting and clearing old sessions.",
        action: "cleanup",
      });
    }
    ...
    if (storageSize > 5 * 1024 * 1024) {
      ...
      recommendations.push({
        type: "warning",
        message:
          "Storage usage is high. Consider enabling compression or cleaning up old data.",
        action: "compress",
      });
    }
  ...
  measureRenderPerformance(messageCount, callback) {
    ...
    // Performance metrics tracking (logging disabled for cleaner console)
    if (renderTime > 100) {
      // More than 100ms is considered slow
      // Slow render detected - logging disabled
    }

...
export class PerformanceMetrics {
  ...
  getSummary() {
    return {
      averageRenderTimes: Array.from(
        this.metrics.componentUpdates.keys()
  ...
  Object.entries(metrics.averageRenderTimes).forEach(([component, avgTime]) => {
    if (avgTime > 16) {
      ...
      recommendations.push({
        ...
        suggestion:
          "Consider memoization, reducing re-renders, or optimizing heavy computations",
      });
    }
  });
  ...
  if (metrics.averageModelLoadTime > 2000) {
    recommendations.push({
      ...
      issue: `Slow model loading (${(
        metrics.averageModelLoadTime / 1000
      ).toFixed(1)}s average)`,
      suggestion:
        "Consider model optimization, compression, or progressive loading",
    });
  }

  // Check memory usage
  if (metrics.memoryTrend > 10 * 1024 * 1024) {
    // 10MB increase
    recommendations.push({
      type: "memory",
      severity: "high",
      ...
      suggestion:
        "Check for memory leaks, dispose unused resources, or implement cleanup",
    });
  }

  // Check component update frequency
  Object.entries(metrics.componentUpdateCounts).forEach(
    ([component, count]) => {
      if (count > 100) {
        recommendations.push({
          type: "updates",
...
export const startPerformanceMonitoring = () => {
  ...
  // Performance summary monitoring (disabled for cleaner console)
